...
const CartModal = () => {
  ...
  const calculateSubtotal = () => {
    if (!cart.lineItems) return 0; // Si no hay elementos, retorna 0

    return cart.lineItems.reduce((total, item) => {
      return total + (Number(item.price?.amount) || 0) * (Number(item.quantity) || 1); // Multiplica el precio por la cantidad
    }, 0);
  };
  ...
  return (
    <div
      ...
      onMouseEnter={() => setIsVisible(true)} // Mostrar el carrito al pasar el mouse
      onMouseLeave={() => setIsVisible(false)} // Ocultar carrito cuando el mouse sale
      style={{ display: isVisible ? 'flex' : 'none' }} // Controlar la visibilidad con display
    >
